...
md"""
Tutorial modified from <https://learn.astropy.org/tutorials/synthetic-images.html>
...
md"""
## Load and examine the FITS file
"""
...
# ╔═╡ e12d49c3-0147-4e46-bb92-d7514beb275f
file_i = download("http://data.astropy.org/tutorials/synthetic-images/synchrotron_i_lobe_0700_150MHz_sm.fits")

# ╔═╡ ffb1d7b7-8ba1-46d1-bc79-71d3ebd38c75
hdulist = FITS(file_i)

# ╔═╡ 1720dc04-e223-451a-aa69-6470320b8ae6
hdu = hdulist["NN_EMISSIVITY_I_LOBE_150.0MHZ"]

# ╔═╡ 3566d676-c72e-4a43-8df4-d51db7c0a075
header = read_header(hdu)

# ╔═╡ 0f234a25-3cdb-4b1f-835f-4fcf0483b21f
md"""
...
# ╔═╡ 2fd80ee1-bed0-4cb0-b737-a4e11c84d548
hdudata = read(hdu)

# ╔═╡ 49263ba2-a9c4-413b-a269-8aa96d32fb1d
maximum(hdudata)

# ╔═╡ aae2255e-50bd-482f-aa80-acec3d48e7bb
minimum(hdudata)

# ╔═╡ 032f4074-a895-43fb-acd9-aa08a110fe7f
hist(
    log10.(hdudata) |> filter(isfinite) |> filter(x -> -3 ≤ x ≤ 2),
    bins = 100,
)

# ╔═╡ 22bcd342-cb22-44c4-93a3-b1c6d0bf0577
md"""
...
# ╔═╡ 424da99c-7daa-4986-80e6-e00942532c4c
md"""
...
# ╔═╡ 0efabced-73cd-4d33-8c16-e7ef97c27985
# distance to the object
dist_obj = 200u"Mpc"

# ╔═╡ a079c29b-51f7-4792-b0b6-55d4445bf27b
begin
...
# ╔═╡ e687ed03-a920-43a8-9cc6-cf5204e21203
md"""
Here we convert the pixel scale from cm to degree by dividing the distance to the object.
"""

# ╔═╡ 6b66a95d-41cf-4ff7-a40d-34ea29efb5f7
cdelt1 = rad2deg(uconvert(NoUnits, header["CDELT1"] * u"cm" / dist_obj)) * u"°"

# ╔═╡ b520e14c-adb7-431c-9a7f-7b59dd8ff9e8
cdelt2 = rad2deg(uconvert(NoUnits, header["CDELT1"] * u"cm" / dist_obj)) * u"°"

# ╔═╡ eb63b7cd-9e4b-42ce-98cc-483392b5c1b3
md"""
Use `astropy.wcs.WCS` to prepare a FITS header.
"""

# ╔═╡ a80451b5-3262-475f-9033-cc2819b7371e


# ╔═╡ 8b1ff476-2cf4-4194-af0a-e22d4f09925e
md"""
Now we can convert the WCS coordinate into header and update the hdu.
"""

# ╔═╡ da3f9d83-de49-4bcd-aeb6-60a477fd12c6


# ╔═╡ 0baef7bf-7b71-4442-8cde-41b5cae00f6e
md"""
Let's take a look at the header. `CDELT1`, `CDELT2`, `CUNIT1`, `CUNIT2`, `CRVAL1`, and `CRVAL2` are in sky coordinates now.
"""

# ╔═╡ f6aafafe-16e3-4412-b746-2148df46052f


# ╔═╡ b4ff8e12-50ab-4916-a675-68509d5c4bf9


# ╔═╡ 8d534f3b-c94c-449e-8c10-494136895a0c
md"""
Now we have the sky coordinate for the image!
"""

# ╔═╡ 0fcd67c4-003a-499b-9cf5-d29d91ec58b8
md"""
## Prepare a Point Spread Function (PSF)
"""

# ╔═╡ 868e449c-ae66-43fe-bc67-3167bb4346fe
md"""
...
# ╔═╡ 1a5ab009-a0ce-4a11-803b-be2b23ae91e5
# assume our telescope has 1 arcsecond resolution
telescope_resolution = 1u"arcsecond"

# ╔═╡ 325a6426-4317-40bb-85e2-e65af31eb893
# calculate the sigma in pixels. Greek σ typed with `\sigma<TAB>`
σ = telescope_resolution / cdelt2 |> NoUnits

# ╔═╡ 2aa657c5-4d03-4bf4-9a4c-43421f594a29
psf = gaussian(x = 0, y = 0, fwhm = 2σ)

# ╔═╡ 9ba88f10-1ef6-4e39-ab4c-7e93b14e054e
let xs = range(-4σ, 4σ, length = 1000)
    heatmap(xs, xs, psf; axis = (; aspect = DataAspect()))
end

# ╔═╡ b651809b-7c70-4f9a-95b3-c05dd4e5b0ac
md"""
### How to do this without astropy kernels
"""

# ╔═╡ dd70bbc6-eecf-4f06-9610-cf2019c1cc58
md"""
Maybe your PSF is more complicated. Here’s an alternative way to do this, using a 2D Lorentzian
"""

# ╔═╡ 6ac12546-242c-4f5c-abdf-0622348559a2
γ = σ; # typed with `\gamma<TAB>`

# ╔═╡ 2b434735-39ee-433e-a1e6-5a326d7b3cd9
x_grid = let
    n = trunc(Int, 8γ)
    range(-4γ, 4γ, length = n) * ones(n)'
end

# ╔═╡ 7afd2348-8c8e-4893-823f-1347bc09be48
r_grid = @. sqrt(x_grid^2 + x_grid'^2)

# ╔═╡ 5f8b5f59-af61-40b2-93ed-e9ad6da1e23c
lorentzian = Cauchy(0, γ)

# ╔═╡ abd4c872-7245-470a-b6d2-62756d04c9ef
# extrude a 2D azimuthally symmetric PSF
begin
    lorentzian_psf = pdf.(lorentzian, r_grid)
    lorentzian_psf ./= sum(lorentzian_psf)
end

# ╔═╡ c7d31477-3da9-4b0d-92c9-331adb0b4ed5
image(lorentzian_psf, axis = (; aspect = DataAspect()), colormap = :viridis)

# ╔═╡ 2daf08cf-0f64-437e-9e95-47a4e23a6dbf
md"""
## Convolve image with PSF
"""

# ╔═╡ d2350ed0-c30d-4d28-9411-752437e5491b
md"""
Here we use [DSP.jl's](https://docs.juliadsp.org) `conv()` to convolve image. This routine uses Fourier transform for faster calculation. Especially since our data is ``2^n`` sized, which makes it particularly fast. Using a FFT, however, causes boundary effects. We’ll need to specify how we want to handle the boundary. Here we choose to “wrap” the data, which means making the data periodic.
"""

# ╔═╡ 8bb74155-a073-45d4-87c3-55fa881df152
convolved_image = conv(hdudata, lorentzian_psf)

# ╔═╡ c3c49a4e-f911-4d28-8a29-e6fb7a5c5cb0
Δx_psf = 100  # number of pixels from the edges

# ╔═╡ f7ee67d3-4e6e-43a5-b2ce-4fceb3eb3f2b
xmin, xmax = -size(lorentzian_psf, 2) - Δx_psf, -Δx_psf

# ╔═╡ 3c14f016-b29b-4126-80b8-a78e7e757164
ymin, ymax = Δx_psf, Δx_psf + size(lorentzian_psf, 1)

# ╔═╡ ee763d94-d331-4201-a683-50f01e6c3f83
md"""
Now let’s take a look at the convolved image.
"""

# ╔═╡ 3b2a7d81-d454-47be-bed6-545c460a6f4d
let
    fig, ax, img = image(
        convolved_image'; colormap = :viridis,
        axis = (; xreversed = true, yreversed = true)
    )
    Colorbar(fig[1, 2], img)
    fig
end

# ╔═╡ f8cb328f-26b4-4ad8-895b-af942a7d810e
md"""
## Convolve Stokes Q and U images
"""

# ╔═╡ 30758a10-114c-4d7e-9d16-93c9a0539de3
hdulist

# ╔═╡ 84cf8413-95f2-4e9a-88c6-ec838f3ac3f9
file_q = download("http://data.astropy.org/tutorials/synthetic-images/synchrotron_q_lobe_0700_150MHz_sm.fits")

# ╔═╡ c4e83f14-381d-44ff-9a86-6c966a6c7c4d
hdulist_q = FITS(file_q)

# ╔═╡ 3e987b71-8a2f-41d9-b07a-6d66932cdaf3
hdu_q = hdulist_q["NN_EMISSIVITY_Q_LOBE_150.0MHZ"]

# ╔═╡ d257da92-060d-458b-a23b-6aae40a51d36
file_u = download("http://data.astropy.org/tutorials/synthetic-images/synchrotron_u_lobe_0700_150MHz_sm.fits")

# ╔═╡ c923a085-74bb-4b73-aae7-6dc309520603
hdulist_u = FITS(file_q)

# ╔═╡ eea09288-bcf8-471d-bf47-dca0bc4f11ef
hdu_u = hdulist_u["NN_EMISSIVITY_Q_LOBE_150.0MHZ"]

# ╔═╡ 375fada9-9ac1-4522-998e-aeaf307a7af2
md"""
Update the header with the wcs_header we created earlier
"""

# ╔═╡ 14973066-e8dc-42ea-8f17-090313973b9d
# TODO

# ╔═╡ 6cee1052-765f-4085-b542-8f91e15e0bed
md"""
Convolve the images with the the psf
"""

# ╔═╡ 7bad4342-0568-4d90-b861-3ea382f852ae
convolved_image_q = conv(read(hdu_q), lorentzian_psf)

# ╔═╡ 4fa1e4d7-6325-49fa-947b-eee47d27755e
convolved_image_u = conv(read(hdu_u), lorentzian_psf)

# ╔═╡ 94c59eea-d169-489e-a58c-16e0beecbd9d
md"""
...
let fig = Figure()
    ax1 = Axis(fig[1, 1]; xlabel = "RA", ylabel = "Dec", aspect = DataAspect())
    im1 = image!(ax1, convolved_image_q, colormap = :seismic)
    Colorbar(fig[1, 2], im1)
    ax2 = Axis(fig[1, 3]; xlabel = "RA", ylabel = "Dec", aspect = DataAspect())
    im2 = image!(ax2, convolved_image_u, colormap = :seismic)
    Colorbar(fig[1, 4], im2)
    fig
end

# ╔═╡ e6971a8b-d017-4719-bb32-9dd9446d47cd
md"""
## Calculate polarization angle and fraction for quiver plot
"""
...
let fig = Figure()
    ...
    i_plot = image!(ax, log10.(convolved_image .+ 1.0e-3), colormap = :viridis)

    # binning factor
    factor = [71, 44]
    # re-binned number of points in each axis
    nx_new = size(convolved_image, 2) ÷ factor[1]
    ny_new = size(convolved_image, 1) ÷ factor[2]

    # These are the positions of the quivers
    x = range(1, size(convolved_image, 1), length = factor[1])
...
git-tree-sha1 = "6da96712db136b31dd7e77cea6d7b75c7ade693a"
