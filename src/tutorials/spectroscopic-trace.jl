...
# ╔═╡ b5586f59-eb18-40fd-b2c7-6eea7d467ea2
using PlutoUI
...
# ╔═╡ 3976089b-0e38-432d-94d4-eb29f933c8bd
spectrum_filepath = download("https://raw.githubusercontent.com/astropy/astropy-tutorials/b2fb15153efc44c14bbc7deab36aec52f58d1367/tutorials/SpectroscopicDataReductionBasics/aldebaran_3s_1.bmp")

# ╔═╡ 07f1c80c-f8ce-4db9-b581-f8cf4dc00382
image_data = load(spectrum_filepath)

# ╔═╡ 71cb2b8b-48a6-4339-b096-3a9666e702d6
typeof(image_data)

# ╔═╡ 96bbbbb3-4fd5-4a1b-8285-89dd79dc8220
eltype(image_data) |> Dump

# ╔═╡ 3a3290ba-35f2-47be-9597-7ca7b696bfee
red.(image_data) == green.(image_data) == blue.(image_data)
...
image_array = rawview(blue.(image_data))

# ╔═╡ 79fd7099-5b53-4b7e-8b5a-f43aea125aa7
let fig = Figure()
    ax = Axis(fig[1, 1])
    image!(ax, image_data') # XXX why is a transpose needed?
    Colorbar(fig[1, 2], limits = extrema(image_array), colormap = :greys)
    fig
end
...
yvals = argmax.(eachcol(image_array))

# ╔═╡ 0f7fd20f-5f27-434b-a030-2422695225e9
xvals = axes(image_data, 2)

# ╔═╡ fb28eae5-60da-4433-afd8-2e5cc7ef21e3
scatter(
    xvals, yvals, marker = :xcross,
...
# ╔═╡ 4246ebc6-ba20-4fc4-b632-45a2653bfb74
md"""
...
let fig = Figure()
    ax = Axis(fig[1, 1], xlabel = "X position", ylabel = "Argmax trace data")
    scatter!(ax, xvals, yvals, marker = :xcross)
    scatter!(ax, xvals[bad_pixels], yvals[bad_pixels], marker = :xcross, color = :red)
    fig
end

# ╔═╡ 1551faad-1ba3-4376-b718-e8a36b37f0e0
scatter(
    xvals[.!bad_pixels], yvals[.!bad_pixels], marker = :xcross,
    axis = (xlabel = "X position", ylabel = "Argmax trace data"),
)
...
scatter(
    xvals[.!bad_pixels_tighter], yvals[.!bad_pixels_tighter], marker = :xcross,
    axis = (xlabel = "X position", ylabel = "Argmax trace data"),
)
...
# ╔═╡ c5019caf-73f9-4aea-addc-ad68c952e876
md"""
...
# ╔═╡ 0ad3b5a1-fa6d-4142-a377-820101df70e6
yaxis = repeat(425:475, 1, size(image_array, 2))

# ╔═╡ f600d2f5-b890-4fef-9be5-e2a8517e96cb
background = median.(eachcol(image_array))

# ╔═╡ a665f59d-b446-4ff3-abb2-5e400b8eead4
weighted_yaxis_values = mean(
    yaxis, weights(image_array[425:475, :] .- background'), dims = 2
)

# ╔═╡ 6118428b-0d08-4eb2-9cf9-fb7b1942ea85
...
# ╔═╡ 8bca64a2-98e0-4068-bde0-17628ae1389d
md"""
...
# ╔═╡ dd5e67dd-0c5c-433e-b4dd-0bc67d7cb0a4


# ╔═╡ b459d888-6ae4-4375-b79d-2e2322034d9f
md"""
...
# ╔═╡ 0ae77f4c-f9a3-4532-bdb1-770226d8c836
md"""
...
# ╔═╡ 04ef1262-e4fd-4961-8702-9915e3a6da28
md"""
...
# ╔═╡ 24afc342-1d77-4e90-9d45-b9cd8bbe77a9
md"""
...
# ╔═╡ f884c80c-033d-4348-9bf0-5cb72c957bd1
TableOfContents()
...
manifest_format = "2.0"
project_hash = "da192311ea1d123df48ba3baac8645377f06721f"
