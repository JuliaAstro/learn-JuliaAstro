...
begin
	...
	# Plots
	using Makie: DQConversion, IntervalsBetween
...
let
	...
	ax = Axis(fig[1, 1];
		dim1_conversion = DQConversion(u"1/μm"),
		xlabel = L"\lambda^{-1} \left(\mu\text{m}^{-1}\right)",
		ylabel = "A(λ) / A(V)",
	)
...
let
	...
	fig, ax, p = lines(wav_spectrum, flux_spectrum_deredden;
		axis = (;
			dim1_conversion = DQConversion(us"Å"),
			dim2_conversion = DQConversion(us"erg/s/cm^2/Å"; units_in_label = false),
			yscale = log10,
...
manifest_format = "2.0"
project_hash = "9a97c3587c2b76ccbbf58f314e087a368362976a"
