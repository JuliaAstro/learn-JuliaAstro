...
# ╔═╡ 65aa0455-9a80-4a4b-b12d-050dbd61b8c5
using PlutoUI
...
# ╔═╡ 642293ad-f90c-488b-ac6c-60054a797086
md"""
This notebook is modified from <https://learn.astropy.org/tutorials/units-and-integration.html>.

Original authors: Zach Pace, Lia Corrales, Stephanie T. Douglas
"""

# ╔═╡ b7a0d11c-632a-45fa-8119-6dfed2a5430b
md"""
!!! tip "Learning Goals"
    ...
    - Gaussian quadrature
    - use Korg.jl's black-body curves
    - understand how Unitful.jl's units interact with one another
    - define a Julia struct type
    - how callable objects works
    - add ``\LaTeX`` labels to Makie.jl figures using the `L"..."` string
"""

# ╔═╡ 27771898-7bf2-474f-a6d0-483d773aeaa8
md"""
!!! note "Keywords"

    modeling, units, synphot, LaTeX, astrostatistics, Makie, units, physics
"""
...
# ╔═╡ d9becd39-bdb6-4736-a8f4-c0fff30afabf
md"""
## Companion Content
...
# ╔═╡ 7cf7adc7-bda1-4d1b-a66a-d668b71c596b
md"""
Required packages for this notebook:
"""

# ╔═╡ 09c719d8-e1a4-4cfc-b4b0-00bb33e316cf
md"""
## The Planck function
"""
...
# ╔═╡ 441c5f17-a05f-45a6-9091-ce0213fb4ac8
md"""
!!! note "Note on printing units"
...
ν = range(1u"THz", 9000u"THz", length = 1000)

# ╔═╡ 49b45c3d-4633-4251-b32d-83fef14d8f21
md"""
!!! todo
    Explain why we're defining two blackbody functions, and how distribution functions depend on arguments. Probably also mention something about multiple dispatch.
"""
...
bb5000K_ν = blackbody.(ν, 5000u"K") .|> u"erg/cm^2"

# ╔═╡ 1da07a0b-612e-488e-94a3-7b2979290efc
let
    fig = Figure()
    ax = Axis(
        fig[1, 1];
    ...
    lines!(ax, ν, bb5000K_ν)

    fig
end

# ╔═╡ 1098259c-1237-4f4e-9256-93b4883fd867
md"""
...
md"""
Here, we’ve used ``\mathrm\LaTeX`` markup to add nice-looking axis labels. To do that, we prefix ``\mathrm\LaTeX`` markup text with a `L`, as a string `L"..."`. The `L` before the open-quote denotes that the string is a ``\mathrm\LaTeX`` string, which we get from Makie.jl and LaTeXStrings.jl. This is the suggested format for axis label text that includes markup.
"""
...
trapz(ν, bb5000K_ν) |> u"erg/(s * cm^2 * sr)"
...
λ = uconvert.(u"μm", ν, Spectral()) |> reverse

# ╔═╡ ff0c4954-0a78-41aa-8694-9b1b7c28b70b
bb5000K_λ = blackbody.(λ, 5000u"K") .|> u"erg/s / (Å * sr * cm^2)"
...
let
    ...
    ax = Axis(
        fig[1, 1];
        xlabel = L"\lambda", ylabel = L"I_\lambda",
        xminorgridvisible = true, yminorgridvisible = true,
        xminorticksvisible = true, yminorticksvisible = true,
        title = "Planck function in wavelength",
        dim1_conversion = Makie.UnitfulConversion(unit(eltype(λ)), units_in_label = true),
        dim2_conversion = Makie.UnitfulConversion(unit(eltype(bb5000K_λ)), units_in_label = false),
    )
    lines!(ax, λ, bb5000K_λ)
    xlims!(ax, -0.1u"μm", 16u"μm")

    fig
end

# ╔═╡ 82786592-f216-431f-895a-31c64dc76332
trapz(λ, bb5000K_λ) |> u"erg/(s * cm^2 * sr)"
...
# ╔═╡ 90dd0d0d-e5a2-4601-bc7b-0d7b88eed18f
md"""
Many `astropy` functions use units and quantities directly. As you gain confidence working with them, consider incorporating them into your regular workflow. Read more [here](https://docs.astropy.org/en/stable/units) about how to use units.
"""
...
# ╔═╡ c00c98bf-e568-4c00-8599-cac48426e456
md"""
...
# ╔═╡ a98a7188-1336-45de-acba-94d681dee4f6
md"""
...
# ╔═╡ 12210b82-b55d-451c-970a-e60e85bfb22c


# ╔═╡ 3f8d9bcf-6b40-4fd5-930d-a429eaa31845
md"""
...
lines(
    ...
    axis = (;
        ...
        xlabel = "Stellar mass [M_⊙]", ylabel = "Probability density",
    )
)

# ╔═╡ 7a5abdca-ec32-402b-a91f-fa187acb48fd
md"""
...
# ╔═╡ 91664437-de9b-441e-972b-8868fd3dc998
md"""
### Where is all the mass?
"""

# ╔═╡ e5e05c6b-8b2f-491f-ba41-86d09a9bad08
md"""
...
and it amounts to weighting the probability density function (the IMF) by mass. More generally, you find the value of some property
that depends on
```math
m^M = ∫_{0.01M_⊙}^{0.6M_⊙} m \, \text{IMF}(m) \, dm
```

by calculating
```math
ρ(m)^M = ∫_{0.01M_⊙}^{0.6M_⊙} ρ(m) \, \text{IMF}(m) \, dm
```
"""
...
# ╔═╡ e086c4e8-342f-45b8-a77f-082df666109a
md"""
### Extras
"""
...
- Derive a relationship between recent star-formation rate and Hα luminosity. In other words, find a value of _C_ for the function
  ```math
  \text{SFR}[\tfrac{M_⊙}{\text{yr}}] = C L_\mathrm{Hα} [\mathrm{\tfrac{erg}{s}}]
  ```
...
- What effect does changing the power-law index or upper mass limit of the IMF have on the value of _C_?

- Predict the effect on the value of _C_ of using a different form of the IMF, like Kroupa or Chabrier (both are lighter on the low-mass end).
...
# ╔═╡ c2cc269b-f90f-4221-8a4a-2164947e104d
md"""
# Notebook setup 🔧
"""
...
manifest_format = "2.0"
project_hash = "1fc45c832441a9fc2317edce24334e395eb7e6f0"
