...
# ╔═╡ ba9e72eb-9f25-4bbe-8384-6632f2a82427
using AstroImages

# ╔═╡ 4fd54737-ba0a-4a3d-b9c0-e973caf646ff
using StatsBase

# ╔═╡ f26efeb1-b20e-4e49-8b20-86ec33a21fc0
...
# ╔═╡ 5d138b1d-cd38-4e92-892e-e2b7885dc9f3
md"""
!!! tip ""
	## Learning Goals
...
# ╔═╡ 8546e828-79d6-4622-b134-15d012b60242
md"""
!!! note ""
	## Keywords
...
# ╔═╡ 04a0ca9b-4632-4a1c-bab1-feb2b8fbfc10
md"""
!!! warning ""
	## Summary
	
	This tutorial demonstrates the use of [Downloads.jl](https://github.com/JuliaLang/Downloads.jl) from base Julia to download a data file, then uses [FITSFiles.jl](https://barrettp.github.io/FITSFiles.jl/dev/) to open the file and [DataFrames.jl](https://dataframes.juliadata.org/) to work with the tabular data. Lastly, we use [PlutoPlotly.jl](https://github.com/JuliaPluto/PlutoPlotly.jl) to visualize the data as 1D and 2D histograms.
"""
...
hdu_list = fits(event_filename)

# ╔═╡ 92892039-a968-46c9-a016-80df289c474c
info(hdu_list)
...
md"""
Now we'll take this data and convert it into a `DataFrames.DataFrame` table:
"""

# ╔═╡ 5b85edcd-6e79-44a0-85a0-eeee58437203
df_evt = DataFrame(hdu_list[2].data)

# ╔═╡ 8ec6647e-b58e-4d9c-9ddf-97cca6da4d75
md"""
!!! note
	While it's possible to access FITS tables directly from the `.data` attribute, wrapping it in a `DataFrame` tends to make a variety of common tasks more convenient.
...
md"""
!!! tip
	We are using the small [PlutoPlotly.jl](https://github.com/JuliaPluto/PlutoPlotly.jl) package for easy interactivity in this notebook format. For high quality plots with a wide range of analysis and customizations, see the modern [Makie.jl](https://makie.org) plotting package.
"""
...
# ╔═╡ 46f24c99-ac83-4204-9342-d6f2c0148e64
plot(df_evt_main;
	x = :x,
	y = :y,
	kind = :histogram2d,
...
# ╔═╡ 8a913898-7b41-42af-815e-ae56132a3b3d
plt = data(df_evt_main) * mapping(:x, :y) * histogram()

# ╔═╡ 484b86eb-895b-4695-88a9-eb1b5c4e25f2
# fg = draw(plt, scales(Color = (; colorscale = log10)))

# ╔═╡ a24d00de-9d06-4f98-ae3b-ca1e6153bd29
h = fit(Histogram, (df_evt_main.x, df_evt_main.y); nbins = (300, 300))

# ╔═╡ fc7f752d-bc9f-4674-98ba-9d94a7ec1f17
h.weights |> extrema

# ╔═╡ c7d93bf3-455d-4ec4-b5ee-03c9df13bad1
h.weights |> extrema

# ╔═╡ e4f2a32b-dcdc-4505-8f31-85015e86c20f
imview(h.weights)
...
manifest_format = "2.0"
project_hash = "5399a05f7510a8f4f4fe42fd09f2c7047142c9a3"
